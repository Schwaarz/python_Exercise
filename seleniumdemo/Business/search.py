...
def sinitekwh_login(username,password):
    ...
    time.sleep(1)
    # driver.find_element_by_xpath(sinitekwhpage.username).clear()
    # driver.find_element_by_xpath(sinitekwhpage.password).clear()
...
def sinitekwh_memu():
    ...
    list = {}
    for i in range(1,8):
        list1 = driver.find_element_by_xpath("/html/body/div[1]/div/div[1]/header/div[1]/div["+str(i)+"]/div[1]/span").text
        list2 = "/html/body/div[1]/div/div[1]/header/div[1]/div["+str(i)+"]/div[1]/span"
        list.update({list1:list2})
    # 悬浮元素
    ActionChains(driver).move_to_element(driver.find_element_by_xpath(list['首页'])).perform()
    time.sleep(5)
    list2_1 = {}
    for a in range(1,10):
        list3 = driver.find_element_by_xpath("/html/body/div[1]/div/div[1]/header/div[1]/div[1]/div[2]/ul/li/ul/li["+str(a)+"]/a/div").text
